...
class TutorServer:
    ...
    def session_timer(self):
        while time.time() < self.session_end_time:
            remaining_time = self.session_end_time - time.time()
            if remaining_time <= 5 * 60 and remaining_time > 5 * 60 - 1:  # 5 minutes left
                self.send_warning_message()
            time.sleep(1)
        self.notify_end_of_session()  # Notifies the students in the session that the session has ended

    # Send a warning message to all students 5 minutes before the session ends
    def send_warning_message(self):
        warning_message = "The session will end in 5 minutes."
        self.broadcast_message(warning_message)
